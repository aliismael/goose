...
/// Render a resource link as text for the model.
///
/// A client sends a link rather than an embedded resource when it cannot inline
/// the contents itself, so read them here when the target is a readable file.
/// A directory, an unreadable file, or a non-file URI still has to be named:
/// dropping the block leaves the model with a sentence that stops exactly where
/// the user pointed at something.
fn read_resource_link(link: &ResourceLink) -> String {
    let path = Url::parse(&link.uri)
        .ok()
        ...
        for block in prompt {
            match block {
                ...
                ContentBlock::ResourceLink(link) => {
                    let text = read_resource_link(link);
...
"#,
    ...
    #[test]
    fn test_read_resource_link_reads_file_contents() {
        let (link, file) = new_resource_link("print(\"hello, world\")").unwrap();

        let result = read_resource_link(&link);
        let expected = format!(
            "
...
```",
    ...
    #[test]
    fn test_read_resource_link_names_directories() {
        let dir = tempfile::tempdir().unwrap();
        let uri = Url::from_file_path(dir.path()).unwrap().to_string();
        let link = ResourceLink::new("some-directory", uri);
    ...
    #[test]
    fn test_read_resource_link_names_unreadable_files() {
        let dir = tempfile::tempdir().unwrap();
        let missing = dir.path().join("missing.txt");
        let uri = Url::from_file_path(&missing).unwrap().to_string();
        let link = ResourceLink::new("missing.txt", uri);
    ...
    #[test]
    fn test_read_resource_link_names_non_file_uris() {
        let link = ResourceLink::new("docs", "https://example.com/docs");
    ...
    #[test]
    fn convert_acp_prompt_keeps_directory_resource_link() {
        let dir = tempfile::tempdir().unwrap();
        let uri = Url::from_file_path(dir.path()).unwrap().to_string();
        let prompt = vec![
            ContentBlock::Text(TextContent::new("Can you tell me what changed in ")),
            ContentBlock::ResourceLink(ResourceLink::new("some-directory", uri)),
        ];
        ...
        assert!(text.contains("Can you tell me what changed in "));
